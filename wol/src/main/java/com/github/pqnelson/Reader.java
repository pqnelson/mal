...
class Reader {
     ...
     */
    public static final Expr readString(final String str) {
    ...
    Expr readForm() {
        ...
        // lists
        case RIGHT_PAREN:
            throw new InputMismatchException("Unexpected ')'");
        case LEFT_PAREN:
    ...
    private String expectedDelimiter(final TokenType rightDelimiter) {
    ...
    ArrayList<Expr> gatherContents(final Token start,
                                   final TokenType leftDelimiter,
                                   final TokenType rightDelimiter) {
        assert (leftDelimiter == start.type)
            : "left delimiter didn't match start token";
        ...
        assert (!isAtEnd())
            : "Somehow I'm at the end, skipping the right delimiter";
        assert (rightDelimiter == peek().type) : "Right delimiter mismatch?";
        next(); // consume the tasty right delimiter, yum yum
        return ast;
    }

    Seq readSeq(final Token start) {
        final ArrayList<Expr> ast
            = gatherContents(start, LEFT_PAREN, RIGHT_PAREN);
        return new Seq(ast);
    }


    Expr readVector(final Token token) {
        return new Vector(gatherContents(token, LEFT_BRACKET, RIGHT_BRACKET));
    }

    Map readMap(final Token token) {
        final ArrayList<Expr> contents
            = gatherContents(token, LEFT_BRACE, RIGHT_BRACE);
    ...
    Expr readList() {
        ...
        assert (LEFT_PAREN == token.type)
            : ("readList expected a left parentheses but found "
               + token.toString());
        return readSeq(token);
    }

    Expr number(final Token token, final Object value) {
